...
public class Evenement implements Serializable {
    ...
    public boolean membrePayed(Membre membre){
        for (int i=0; i<this.getListeEquipes().size(); i++){
            for (int j=0; j<this.getListeEquipes().get(i).getListeMembres().size(); j++){
                EquipeMembre em = this.getListeEquipes().get(i).getListeMembres().get(j);
                if (em.getMembre().getId() == membre.getId() && em.isPay())
                    return true;
            }
        }
        return false;
